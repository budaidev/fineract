...
public final class ProgressiveEMICalculator implements EMICalculator {
    ...
    /**
     * Summarize Fn values for Full Term Tranche calculations. The fnResult formula skips the first period (its rate
     * factor is only used in rateFactorN). So this method is identical to calculateFnResult - the filtering only
     * applies to rateFactorN calculation, not fnResult.
     */
    private BigDecimal calculateFnResultFromDate(final List<RepaymentPeriod> periods, final LocalDate fromDate, final MathContext mc) {
        // fnResult calculation skips the first period - filtering only applies to rateFactorN
        return calculateFnResult(periods, mc);
    }
